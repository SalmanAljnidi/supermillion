...
function newQuestion() {
    ...
    // --- المنطق الجديد: 50% ضرب و 50% قسمة ---
    const isMultiplication = Math.random() > 0.5;
    
    if (isMultiplication) {
        // ضرب: الأعداد من 2 إلى 12
        const num1 = Math.floor(Math.random() * 11) + 2; // 2-12
        const num2 = Math.floor(Math.random() * 11) + 2; // 2-12
        correctVal = num1 * num2;
        // الصيغة: ٥ × ٧ = ؟
        document.getElementById('question-text').innerText = `${toArabic(num1)} × ${toArabic(num2)} = ؟`;
    } else {
        // قسمة: المقسوم عليه (2-12) والناتج (2-12)
        const divisor = Math.floor(Math.random() * 11) + 2; // المقسوم عليه
        const result = Math.floor(Math.random() * 11) + 2;  // الناتج
        const dividend = divisor * result; // المقسوم
        correctVal = result;
        // الصيغة: ٣٠ ÷ ٥ = ؟
        document.getElementById('question-text').innerText = `${toArabic(dividend)} ÷ ${toArabic(divisor)} = ؟`;
    }
    ...
    for(let i=0; i<4; i++) {
        if(i !== correctIndex) {
            let w;
            do {
                // توليد إجابة خاطئة قريبة من الإجابة الصحيحة
                const offset = Math.floor(Math.random() * 10) + 1;
                const sign = Math.random() > 0.5 ? 1 : -1;
                w = correctVal + (offset * sign);
                
                // التأكد أن الإجابة الخاطئة موجبة وليست مكررة
                if (w <= 0 || w === correctVal || answers.includes(w)) {
                     w = Math.floor(Math.random() * 144) + 1; // رقم عشوائي احتياطي
                }
            } while(w === correctVal || answers.includes(w));
            answers[i] = w;
        }
    }
        ...
        if(idx === correctIndex) {
            ...
            setTimeout(() => {
                ...
                if(currentLevel >= prizes.length) {
                    showModal('🎉 مبروك المليون 🎉', `ألف مبروك يا ${playerName}! أنت عبقري الرياضيات وحصلت على المليون!`);
            ...
            playSound('wrong');
            setTimeout(() => {
                showModal('حظاً أوفر', `للأسف إجابة خاطئة يا ${playerName}. الجواب الصحيح هو ${toArabic(correctVal)}`);
                currentLevel = 0;
...
function usePhone() {
    ...
    setTimeout(() => { document.getElementById('modal-content').innerHTML = `<div style="font-size:40px">🤠</div><p>هلا والله يا <b>${playerName}</b>!<br>أعتقد أن الجواب الصحيح هو <b>${toArabic(correctVal)}</b></p>`; }, 3000);
}
